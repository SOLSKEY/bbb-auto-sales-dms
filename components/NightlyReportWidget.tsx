...
const NightlyReportWidget: React.FC = () => {
    ...
    const fetchReportData = async () => {
        ...
        try {
            ...
            statusLogs.forEach(log => {
                ...
                } else if (log.previous_status === 'Repairs' && log.new_status === 'Available') {
    ...
    return (
        <>
            {/* Floating Action Button - Glass Card Accent */}
            <button
                onClick={() => setIsOpen(true)}
                onMouseEnter={() => setIsHovered(true)}
                onMouseLeave={() => setIsHovered(false)}
                className="glass-card-accent rounded-full p-4 transition-all duration-300 flex items-center justify-center relative overflow-hidden"
                style={{
                    position: 'fixed',
                    bottom: '24px',
                    right: '24px',
                    ...
                    // Inner fill gradient - more cyan/blue showing
                    backgroundImage: isHovered
                        ? 'radial-gradient(circle at center, rgba(34, 211, 238, 0.8) 0%, rgba(6, 182, 212, 0.6) 40%, rgba(6, 182, 212, 0.4) 70%, rgba(6, 182, 212, 0.2) 100%), radial-gradient(ellipse at top left, rgba(6, 182, 212, 0.4), transparent 70%), linear-gradient(180deg, rgba(255, 255, 255, 0.08) 0%, rgba(0, 0, 0, 0.3) 100%)'
                        : 'radial-gradient(ellipse at top left, rgba(6, 182, 212, 0.5), transparent 70%), linear-gradient(180deg, rgba(255, 255, 255, 0.08) 0%, rgba(0, 0, 0, 0.3) 100%)',
                    backgroundBlendMode: 'overlay',
                }}
                aria-label="Open Nightly Report"
            >
                {/* Interior glow overlay on hover - bright neon cyan with smooth gradient */}
                {isHovered && (
                    <>
                        {/* Base bright cyan layer with smooth gradient stops - using #00c4e2 hue */}
                        <div 
                            className="absolute inset-0 rounded-full pointer-events-none"
                            style={{
                                background: 'radial-gradient(circle at center, rgba(0, 196, 226, 1) 0%, rgba(0, 196, 226, 0.98) 8%, rgba(0, 196, 226, 0.95) 15%, rgba(0, 196, 226, 0.9) 22%, rgba(0, 196, 226, 0.8) 30%, rgba(0, 196, 226, 0.65) 40%, rgba(0, 196, 226, 0.5) 50%, rgba(0, 196, 226, 0.35) 60%, rgba(0, 196, 226, 0.2) 70%, rgba(0, 196, 226, 0.1) 80%, rgba(0, 196, 226, 0.05) 90%, transparent 100%)',
                                mixBlendMode: 'screen',
                                zIndex: 1,
                            }}
                        />
                        {/* Additional bright center highlight for intense light effect */}
                        <div 
                            className="absolute inset-0 rounded-full pointer-events-none"
                            style={{
                                background: 'radial-gradient(circle at center, rgba(0, 196, 226, 1) 0%, rgba(0, 196, 226, 0.95) 12%, rgba(0, 196, 226, 0.85) 20%, rgba(0, 196, 226, 0.7) 30%, rgba(0, 196, 226, 0.4) 45%, transparent 65%)',
                ...
                <ClipboardDocumentIcon 
                    className="h-6 w-6 relative transition-colors duration-300"
                    style={{
                        color: isHovered ? '#000000' : '#06b6d4',
                        zIndex: 2,
                    }}
                />
            </button>
...
export default NightlyReportWidget;
